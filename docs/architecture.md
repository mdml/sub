...
| Crate | Kind | Role |
|:--|:--|:--|
| `sub-sdk` | library | The kernel: the delegated-work model and the SDK. Owns the shared ACP client layer (`sub_sdk::acp`). The only crate that depends on the ACP SDK crate directly. |
...
| `sub-harness-fake` | binary `sub-harness-fake` | Programmable fake harness (ACP v1 over stdio) for the behavioral contract suite. Depends on `sub-sdk`. |
...
The shared ACP client layer in `sub-sdk` spawns an agent process, negotiates protocol v1, opens a session, sends a prompt, consumes the update stream, cancels, and times out. The fake harness in `sub-harness-fake` is a child process that replays fixture streams and is scriptable per scenario; the contract suite in `sub-sdk` drives both through the same client API (fake harness in CI; real harnesses opt-in via `SUB_CONTRACT_REAL_HARNESS`).
