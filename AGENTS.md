...
- `stable` is assembled by the `promote to stable` workflow from a nightly-tagged `main` commit plus explicitly ordered cherry-picks. Its ruleset permits only the GitHub Actions integration to create, update, or delete the branch; humans have no bypass.
- Humans never create release tags. The nightly and stable promotion workflows create tags, and the release-tag ruleset permits only the GitHub Actions integration to write matching tags.
